...
export const Modal: React.FC<ModalProps> = ({
  ...
  // ล็อกไม่ให้ scroll หน้าหลักเมื่อ modal เปิด
  useEffect(() => {
    if (isOpen) {
            ...
            {(title || showCloseButton) && (
              <div className="flex items-center justify-between px-4 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4 border-b border-slate-100">
                ...
                {showCloseButton && (
                  <button
                    onClick={onClose}
                    className="text-slate-400 hover:text-slate-600 hover:bg-slate-100 p-1.5 rounded-full transition-colors focus:outline-none"
